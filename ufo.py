...
def rotate(strip, color, wait_ms=10, iterations): #light legs and windows sequentially going around the UFO
    for i in range(iterations):
        leg(strip, i%5, color)
        leg(strip, (i-1)%5, Color(0,0,0))
        strip.show()
        time.sleep(wait_ms/1000.0)
        window(strip, i%5, color)
        window(strip, (i-1)%5, Color(0,0,0))
        strip.show()
        time.sleep(wait_ms/1000.0)
    ...
    try:
        ...
        for number in range(5):
            ...
            for p in range(25): #legs
                X[114-p+50*number] = (3-0.07*p)*math.sin(number*math.pi()*2/5)-(0.008*p)*math.cos(number*math.pi()*2/5)
                Y[114-p+50*number] = (3-0.07*p)*math.cos(number*math.pi()*2/5)+(0.008*p)*math.sin(number*math.pi()*2/5)
                Z[114-p+50*number] = 0.07*p
                X[114+p+50*number] = (3-0.07*p)*math.sin(number*math.pi()*2/5)+(0.008*p)*math.cos(number*math.pi()*2/5)
                Y[114+p+50*number] = (3-0.07*p)*math.cos(number*math.pi()*2/5)-(0.008*p)*math.sin(number*math.pi()*2/5)
                Z[114+p+50*number] = 0.07*p
                X[340+p+32*number] = (3-0.075*p)*math.sin(number*math.pi()*2/5)
                Y[340+p+32*number] = (3-0.075*p)*math.cos(number*math.pi()*2/5)
            ...
            for p in range(18): #windows
                ...
                v = math.arctan(1/2)
                X[p+18*number] = (1.25*math.cos(v)+0.3*
            
            for p in range(69): #great circles
                
        for p in range(79): #equator (only one of these)
            
            X[845+p] = 2.5*math.sin(math.pi()*p/7.85*2)
            Y[845+p] = 2.5*math.cos(math.pi()*p/7.85*2)
            Z[845+p] = 3.5
